...
describe("SettingsPage", () => {
  ...
  it("shows validation error when quick link has URL but missing labels", async () => {
    ...
    await waitFor(() => expect(mockGetSettings).toHaveBeenCalled());
    await user.click(screen.getByRole("button", { name: /add link|add_link/i }));
    const urlInputs = screen.getAllByPlaceholderText("URL");
    await user.type(urlInputs[urlInputs.length - 1], "https://example.com");
    await user.click(screen.getByRole("button", { name: /save/i }));
    await waitFor(() => {
  ...
  it("shows validation error when store name (EN) exceeds 200 characters", async () => {
    const user = userEvent.setup();
    mockGetSettings.mockResolvedValue({
      data: {
        settings: {
  ...
  it("shows 'Please fix the errors below' when validation fails", async () => {
    const user = userEvent.setup();
    mockGetSettings.mockResolvedValue({
      data: {
        settings: {
