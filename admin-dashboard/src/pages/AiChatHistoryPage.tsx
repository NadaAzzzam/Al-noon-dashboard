...
const AiChatHistoryPage = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = (await api.getAiSession(viewingId)) as {
          data?: {
            id: string;
            sessionId: string;
            messages: ChatMessage[];
        ...
        >
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexShrink: 0 }}>
              <h2 style={{ margin: 0 }}>{t("ai_chats.view")} — {detail ? `${detail.sessionId.slice(0, 24)}…` : "…"}</h2>
            ...
            {detail ? (
              <>
                {(detail.customerName || detail.customerEmail) && (
                  <p style={{ marginBottom: 12, color: "var(--text-secondary)", flexShrink: 0 }}>
