...
router.get("/sessions", authenticateAdmin, requirePermission(["ai_chats.view"]), validate(listSessionsQuerySchema), listSessions);
/** Admin: get one session with messages */
router.get("/sessions/:id", authenticateAdmin, requirePermission(["ai_chats.view"]), validate(sessionIdParamSchema), getSessionById);
/** Admin: delete a session */
router.delete("/sessions/:id", authenticateAdmin, requirePermission(["ai_chats.manage"]), validate(sessionIdParamSchema), deleteSession);
