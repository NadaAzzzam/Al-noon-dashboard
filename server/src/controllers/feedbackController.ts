...
export const getFeedback = asyncHandler(async (req, res) => {
  ...
  if (!item) throw new ApiError(404, "Feedback not found", { code: "errors.feedback.not_found" });
  sendResponse(res, req.locale, { data: { feedback: item } });
});
...
export const createFeedback = asyncHandler(async (req: Request, res) => {
  ...
  if (!product) throw new ApiError(400, "Product is required", { code: "errors.feedback.product_required" });
  if (!customerName) throw new ApiError(400, "Customer name is required", { code: "errors.feedback.customer_name_required" });
  if (!message) throw new ApiError(400, "Message is required", { code: "errors.feedback.message_required" });
  if (rating == null || rating < 1 || rating > 5) throw new ApiError(400, "Rating must be between 1 and 5", { code: "errors.feedback.invalid_rating" });
  const productExists = await Product.findById(product).select("_id").lean();
  if (!productExists) throw new ApiError(400, "Product not found", { code: "errors.product.not_found" });
  ...
  sendResponse(res, req.locale, { status: 201, message: "success.feedback.created", data: { feedback: populated ?? doc } });
});
...
export const updateFeedback = asyncHandler(async (req: Request, res) => {
  ...
  if (!doc) throw new ApiError(404, "Feedback not found", { code: "errors.feedback.not_found" });
  ...
  if (body.rating !== undefined) {
    ...
    if (r < 1 || r > 5) throw new ApiError(400, "Rating must be between 1 and 5", { code: "errors.feedback.invalid_rating" });
    doc.rating = r;
  }
  ...
  await doc.save();
  const populated = await ProductFeedback.findById(doc._id).populate("product", "name").lean();
  sendResponse(res, req.locale, { message: "success.feedback.updated", data: { feedback: populated ?? doc } });
});
...
export const setFeedbackApproved = asyncHandler(async (req, res) => {
  ...
  if (!doc) throw new ApiError(404, "Feedback not found", { code: "errors.feedback.not_found" });
  sendResponse(res, req.locale, { message: "success.feedback.status_updated", data: { feedback: doc } });
});
...
export const uploadFeedbackImage = asyncHandler(async (req: Request, res) => {
  ...
  sendResponse(res, req.locale, { data: { image: pathUrl } });
});
